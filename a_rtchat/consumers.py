...
class ChatroomConsumer(WebsocketConsumer):
    def connect(self):
        ...
        #дабовление пользователя что он в онлайн
    ...
    def message_handler(self, event):
        self.send(text_data=json.dumps({"body": event['body'], "author": event['author']}))
    ...
    def online_count_handler(self, event):
        ...
        #json.dumps() — это функция из модуля json в Python, которая используется для преобразования (сериализации) объектов Python в строку формата JSON (JavaScript Object Notation).
        self.send(text_data=json.dumps({"online_count": online_count}))
